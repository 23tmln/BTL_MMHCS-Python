...
async def generate_keys_for_user(user_id: str) -> Dict[str, Any]:
    """Generate encryption keys for a user via crypto service"""
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                f"{CRYPTO_SERVICE_URL}/generate-keys",
                json={"userId": user_id}
            )
...
async def get_public_bundle(user_id: str) -> Optional[Dict[str, Any]]:
    """Get public key bundle for a user from crypto service"""
    async with httpx.AsyncClient() as client:
        try:
...
async def encrypt_message(sender_id: str, receiver_id: str, plaintext: str, recipient_bundle: Dict[str, Any]) -> Dict[str, Any]:
    """Encrypt a message using crypto service"""
    async with httpx.AsyncClient() as client:
        try:
            payload = {
                "from": sender_id,
                "to": receiver_id,
                "plaintext": plaintext,
                "recipientBundle": recipient_bundle
            }
            response = await client.post(
...
async def decrypt_message(sender_id: str, receiver_id: str, ciphertext: str, message_type: str, session_id: str) -> Dict[str, Any]:
    """Decrypt a message using crypto service"""
    async with httpx.AsyncClient() as client:
        try:
            payload = {
                "from": sender_id,
...
async def backup_user_state(user_id: str, pin: str) -> Dict[str, Any]:
    """Request crypto service to backup user crypto state."""
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                f"{CRYPTO_SERVICE_URL}/secure-storage/backup",
                json={"userId": user_id, "pin": pin}
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
...
async def restore_user_state(user_id: str, pin: str, encryptedState: str, salt: str, iv: str, authTag: str) -> Dict[str, Any]:
    """Request crypto service to restore user crypto state from encrypted blob."""
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                f"{CRYPTO_SERVICE_URL}/secure-storage/restore",
                json={
                    "userId": user_id,
                    "pin": pin,
...
async def secure_storage_status(user_id: str) -> Dict[str, Any]:
    """Check secure storage status from crypto service (optional)."""
    async with httpx.AsyncClient() as client:
        try:
